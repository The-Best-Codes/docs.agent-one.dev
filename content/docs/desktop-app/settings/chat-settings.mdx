...
- **Assistant messages only** - only assistant responses render markdown.
...
When enabled (default), AgentOne repairs incomplete formatting while a response is still streaming. Disable it if you prefer to see the provider's raw partial markdown or want to remove its small rendering overhead.
...
Shows a button that quickly returns you to the latest message when you have scrolled up. It is enabled by default.

### Message Navigation Rail

Shows message previews on the right side of chats and lets you jump directly to a message. The rail marks messages currently in view, is enabled by default, and stays hidden in the compact layout.
